...
function showDetails(id) {
  ...
  document.getElementById("modalBody").innerHTML = `
        <div class="detail-item"><div class="detail-label">📜 الترتيب</div><div>النبي رقم ${p.order}</div></div>
        <div class="detail-item"><div class="detail-label">👥 القوم</div><div>${p.people}</div></div>
        <div class="detail-item"><div class="detail-label">✨ المعجزات</div><div>${p.miracles}</div></div>
        <div class="detail-item"><div class="detail-label">📖 نبذة</div><div>${p.desc}</div></div>
        <div class="detail-item"><div class="detail-label">⭐ المكانة</div>
          <div><span class="status-tag ${p.category}">${p.category === "ululazm" ? "من أولي العزم من الرسل" : "نبيٌّ مرسل إلى قومه"}</span></div>
        </div>`;
